...
local lvpoke = xsalada.level+boost
local tagap =  getItemAttribute(item.uid, "boost")
local levellife = ((getPlayerLevel(cid)+boost) *xsalada.cons)
if candy then
levellife = ((getPlayerLevel(cid)+boost+(candy*3)) *xsalada.cons)
tagap =  getItemAttribute(item.uid, "boost")+candy
lvpoke = xsalada.level+boost+(candy*3)
end
...
local tmdp = TM[TMFind(item.uid)]
if tmdp then
setPlayerStorageValue(pk,1236,tmdp.minLv+xsalada.level)
end
for rocks = 1, 12 do 
...
local niveldopoke = xsalada.level
