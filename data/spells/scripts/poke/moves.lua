...
local magexe = {"hypnosis", "scary face","sleep powder", "stun spore", "absorb", "reflect", "restore", "rest", "self heal", "sing", "melody", "healera", "roar"}
...
local pokemon2 = cp[nick]
if not pokemon2 then
local nickshiny = nick
...
local magia = cp[nick][fala]
if not magia then
return true
end
if magia.spell == "Psy Condition" then
return true
end
...
local tspell = string.upper (magia.spell)
doCreatureSay(cid,  ""..tspell .."!!!", TALKTYPE_MONSTER)
...
if not isMonster(getCreatureTarget(cid)) and not isPlayer(getCreatureTarget(cid)) then
	...
	elseif magia.type == "dragon" then
		elementa = DRAGONDAMAGE
			elseif magia.type == "dark" then
		elementa = DARKDAMAGE
	elseif magia.type == "steel" then
...
local leveldoplayer = pokes[nick].level
...
local exausted = true
if getCreatureMaxHealth(cid) >= 30000 then
exausted = false
end
if isInArray(magexe, string.lower(magia.spell)) then 
exausted = true
end
if exausted then
...
local checklife = getCreatureMaxHealth(cid)
local antigonum3 = num3
if isStrong(getCreatureName(cid))  then
num3 = num3 * math.random(1,5)
elseif checklife >= 200000 then
num3 = num3 * math.random(1,3)
elseif checklife >= 100000 then
num3 = num3 * math.random(1,2)
end
if num3 > antigonum3 then
doSendAnimatedText(getThingPos(cid), "Critical", 215) 
end
...
elseif getCreatureName(cid) == "Donphan" then
		doSetCreatureOutfit(cid, {lookType = 586}, 8300)
elseif getCreatureName(cid) == "Phanpy" then
		doSetCreatureOutfit(cid, {lookType = 656}, 8300)
elseif getCreatureName(cid) == "Shiny Miltank" then
doSetCreatureOutfit(cid, {lookType = 1078}, 8300)
elseif getCreatureName(cid) == "Shiny Phanpy" then
doSetCreatureOutfit(cid, {lookType = 1080}, 8300)
elseif getCreatureName(cid) == "Shiny Donphan" then
doSetCreatureOutfit(cid, {lookType = 1086}, 8300)
