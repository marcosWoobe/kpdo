...
doPlayerSendCancel(cid,"You cant watch tv, fly/ride")
return false
end


if isTV(cid) then
doPlayerSendCancel(cid,"you can not watch TV when you have to open TV")
return false
end
...
setPlayerStorageValue(cid, 91922,getPlayerStorageValue(playertv,91919))
doSendChannelPVT(playertv,getCreatureName(cid))
doSendChannel(cid,getPlayerStorageValue(playertv,91919),getCreatureName(playertv))
setPlayerStorageValue(cid,91923,getCreatureName(playertv))
local function tvmode(params)
if isCreature(params.cid) then
local playertv = getPlayerStorageValue(params.cid, 91911)
if isPlayer(playertv)  then
local poscam = getCreaturePosition(playertv)
if getCreaturePosition(params.cid) == poscam then
addEvent(tvmode, 100, {cid = params.cid})
return true
else
doTeleportThing(params.cid, poscam,false)
addEvent(tvmode, 100, {cid = params.cid})
return true
end
end
...
addEvent(tvmode, 100, {cid = cid})
for x = 0, 100 do
local player = getPlayerStorageValue(cid,1746190+x)
if player >= 1 then
setPlayerStorageValue(cid,1747190+x,nil)
setPlayerStorageValue(cid,1746190+x,nil)
end
