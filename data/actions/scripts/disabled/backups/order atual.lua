...
local headbutt = {'Venusaur', 'Ivysaur', 'Bulbasaur', 'Shiny Venusaur', 'Dratini', 'Shiny Dratini', 'Dragonair', 'Shiny Dragonair', 'Dragonite', 'Shiny Dragonite', 'Shiny Arcanine','Caterpie','Arcanine','Metapod','Weedle','Kakuna','Rattata','Raticate','Squirtle','Wartortle','Blastoise','Shiny Blastoise','Slowpoke','Slowbro','Gyarados','Growlithe','Spearow','Fearow','Pidgey','Pidgeotto','Pidgeot','Pikachu','Raichu','Shiny Raichu','Meowth','Persian','Machamp','Machoke','Machop','Primeape','Mankey','Seel','Dewgong','Cubone','Marowak','Lickitung','Koffing','Weezing','Kangaskhan','Tauros','Eevee','Aerodactyl'}
...
if isCreature(item2.uid) and getPlayerStorageValue(cid, 63215) <= 0 and getPlayerStorageValue(cid, 17000) <= 0 and item2.uid ~= cid then
...
doCreatureSay(cid, ""..getCreatureName(getCreatureSummons(cid)[1])..", control "..getCreatureName(pk).."'s mind!", 1)
local cmed = pk
...
doRemoveCreature(getCreatureSummons(cid)[1])
local pk = doSummonCreature(getItemAttribute(item.uid, "poke"):match("This is (.-)'s pokeball."), mynewpos) 
doConvinceCreature(cid, pk)
doCreatureSetLookDir(getCreatureSummons(cid)[1], 2)
registerCreatureEvent(pk, "DiePoke")
registerCreatureEvent(pk, "Exp2")
...
doChangeSpeed(getCreatureSummons(cid)[1], -getCreatureSpeed(getCreatureSummons(cid)[1]))
	...
	if badire == NORTHWEST then
		if math.random(1,100) >= 51 then
		badire = NORTH
			if isWalkables(getPosByDir(getThingPos(getCreatureSummons(params.cid)[1]), badire), params.ccid, 0, 0) then
			badire = NORTH
			else
			badire = WEST
			end
...
setPlayerStorageValue(cid, 1951, 0)
	local function tocut(params)
	if isCreature(params.cid) then
	if isCreature(getCreatureSummons(params.cid)[1]) then
	...
	if getDistanceBetween(getThingPos(getCreatureSummons(params.cid)[1]), topos) == 1 then
if getPlayerStorageValue(cid, 1951) == 1 then
return true
end
setPlayerStorageValue(cid, 1951, 1)
if not isInArray({2704, 2705, 2701, 2703, 2706}, getTileThingByPos(topos).itemid) then
...
doChangeSpeed(getCreatureSummons(cid)[1], -getCreatureSpeed(getCreatureSummons(cid)[1]))
	...
	if adire == NORTHWEST then
		if math.random(1,100) >= 51 then
		adire = NORTH
			if isWalkables(getPosByDir(getThingPos(getCreatureSummons(params.cid)[1]), adire), params.ccid, 0, 0) then
			adire = NORTH
			else
			adire = WEST
			end
...
doChangeSpeed(getCreatureSummons(cid)[1], -getCreatureSpeed(getCreatureSummons(cid)[1]))
	...
	if cdire == NORTHWEST then
		if math.random(1,100) >= 51 then
		cdire = NORTH
			if isWalkables(getPosByDir(getThingPos(getCreatureSummons(params.cid)[1]), cdire), params.ccid, 0, 0) then
			cdire = NORTH
			else
			cdire = WEST
			end
...
local function move(params)
	...
	if edire == NORTHWEST then
		if math.random(1,100) >= 51 then
		edire = NORTH
			if isWalkables(getPosByDir(getThingPos(getCreatureSummons(params.cid)[1]), edire), params.ccid, 0, 0) then
			edire = NORTH
			else
			edire = WEST
			end
...
if not isWalkables(topos, cid, 0, 0) then
...
doChangeSpeed(viadao,getPlayerStorageValue(viadao, 2997) ) 
setPlayerStorageValue(viadao, 1869, 0)
setPlayerStorageValue(viadao, 2997, 0)
...
if params.i == params.travel then

doPlayerSay(cid, ""..getCreatureName(getCreatureSummons(params.cid)[1])..", Sux!", 1)
doChangeSpeed(getCreatureSummons(params.cid)[1], -getCreatureSpeed(getCreatureSummons(params.cid)[1]))
setPlayerStorageValue(getCreatureSummons(cid)[1], 9951, 1)
setPlayerStorageValue(getCreatureSummons(cid)[1], 1869, 0) 
setPlayerStorageValue(getCreatureSummons(params.cid)[1], 1099, 0)
...
addEvent(move, temk*i, {cid = cid, ccid = getCreatureSummons(cid)[1], topos = pois, item2 = item2.uid,travel = travel,i = i})
end
end
