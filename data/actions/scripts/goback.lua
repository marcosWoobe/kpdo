...
gobackeff = 223
diseff = 49
elseif item.itemid == 11215 or item.itemid == 11213 then
gobackeff = 196
diseff = 52
end
local effect = gobackeff

if item.itemid == 2223 or item.itemid == 2221 or item.itemid == 2538 or item.itemid == 2539 or item.itemid == 190 or item.itemid == 196 or item.itemid == 199 or item.itemid == 11215 then
...
local mbk = msgunicaback
if frases and frases ~= "null" then
else
mbk = msgunicaback:gsub("doka", apelido2)
end
...
elseif item.itemid == 11215 then
doTransformItem(item.uid, 11213)
end
...
if pokename == "Ditto" or pokename == "Smeargle"  then
...
local npcnome = getPlayerStorageValue(cid, 6578)
if npcnome == "Lorelei" then
doTeleportThing(npc, {x= 538,y= 997,z= 3})
end
if getPlayerStorageValue(cid, 8334) <= 0 then
...
return doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Cant use go you need a leave flying/surf/ride")
end
...
elseif item.itemid == 2222 or item.itemid == 2220 or item.itemid == 2653 or item.itemid == 2652 or item.itemid == 198 or item.itemid == 2200 or item.itemid == 189 or item.itemid == 11213 then
...
local pokesloot = getItemAttribute(item.uid, "nome")
					local items = getItemsInContainerByIds(bp.uid, {'2652','2224','2541','2542','2227','189','191','2200','197','198','200','2222','2653','2220','11213','11214'})    	
...
if getItemAttribute(item.uid, "sexy")  == 4 then
levellife = levellife * 1.1
end
...
local mgo = msgunicago
if frasesgo and frasesgo ~= "null" then

else
mgo = msgunicago:gsub("doka", pokenick)
end
...
local tmp = TM[TMFind(item.uid)]
if tmp then
setPlayerStorageValue(pk,1236,tmp.minLv+x.level)
setPlayerStorageValue(cid,451236,tmp.minLv+x.level)
end
for rocks = 1, 12 do 
...
local sexypokemon = getItemAttribute(item.uid, "sexy")
if sexypokemon then
doCreatureSetSkullType(pk, sexypokemon)
else
local randomsexy = math.random(4,5)
doItemSetAttribute(item.uid, "sexy", randomsexy)
doCreatureSetSkullType(pk, randomsexy)
end
if getPlayerStorageValue(cid, 8888) >= 1 then
